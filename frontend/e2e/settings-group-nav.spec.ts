...
async function setup(page: Page, options: SetupOptions = {}) {
  ...
  await page.route(`${API_BASE}/workspaces/**`, async (route) => {
    ...
    // Gmail exclusions renders `data.rules.length` / `data.audit.length`
...
/**
 * The four pages that had no way back: no sidebar entry, no breadcrumbs, no
 * link to their parent. Each now sits under a shared area header.
 */
const GROUPS = [
  {
    area: "Access control",
    tabs: ["Overview", "Templates", "Access logs", "Gmail exclusions"],
    pages: [
      ["/settings/access", "Overview"],
      ["/settings/access/templates", "Templates"],
      ["/settings/access/logs", "Access logs"],
      ["/settings/access/gmail-exclusions", "Gmail exclusions"],
    ],
  },
  {
    area: "Identity",
    tabs: ["Commit ownership", "Ghost developers"],
    pages: [
...
for (const group of GROUPS) {
  for (const [path, expected] of group.pages) {
    test(`${path} offers a way out`, async ({ page }) => {
      await setup(page);
      await page.goto(path);

      const nav = page.getByRole("navigation", { name: group.area });
      await expect(nav).toBeVisible({ timeout: 20000 });

      // Every sibling reachable from every sibling.
      for (const label of group.tabs) {
        await expect(nav.getByRole("link", { name: label })).toBeVisible();
      }
      await expect(nav.locator('[aria-current="page"]')).toHaveText(expected);

      // And a breadcrumb trail that leads somewhere, which is what these
      // pages were missing entirely.
      const crumbs = page.getByRole("navigation", { name: "Breadcrumb" });
      await expect(crumbs.getByRole("link", { name: "Settings" })).toBeVisible();
      if (path !== group.pages[0][0]) {
        await expect(
          crumbs.getByRole("link", { name: group.area }),
        ).toBeVisible();
      }
    });
  }
