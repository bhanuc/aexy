...
async def test_known_partner_domain_assigns_its_mapped_kam_without_ai(db_session: AsyncSession):
    ...
    partner = ServiceDeskPartner(workspace_id=ws.id, name="ABC Finance", assigned_kam_id=mapped)
    db_session.add(partner)
    await db_session.flush()
    db_session.add(
        ServiceDeskPartnerDomain(
            workspace_id=ws.id, partner_id=partner.id, domain="abcfinance.com"
        )
    )
    ...
    assert sd.partner_id == partner.id, "partner is filled from the sender's domain"
    ...
    assert sd.lob_id is None, "the LOB is the KAM's to set by hand in free mode"
