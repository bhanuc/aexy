...
Usage:
    docker exec aexy-backend python scripts/diagnose_pr_commits.py \\
        --repo bimaplan/codebase-v2 --pr 2973

Multiple PRs at once:
    docker exec aexy-backend python scripts/diagnose_pr_commits.py \\
        --repo bimaplan/codebase-v2 --pr 2973 --pr 2914 --pr 2940

Pipe-form (no file deploy needed):
    cat scripts/diagnose_pr_commits.py | docker exec -i aexy-backend \\
        python - --repo bimaplan/codebase-v2 --pr 2973
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--repo",
        required=True,
        help="owner/name of the repository (e.g., bimaplan/codebase-v2)",
    )
    parser.add_argument(
        "--pr",
        type=int,
